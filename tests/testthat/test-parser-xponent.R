...
test_that("Parse the random plate data", {
  plate_file <- system.file("extdata", "random.csv", package = "SerolyzerR", mustWork = TRUE)
  expect_error(read_xponent_format(plate_file), NA)
})

test_that("Parse the random plate 2 data", {
  plate_file <- system.file("extdata", "random2.csv", package = "SerolyzerR", mustWork = TRUE)
  expect_error(read_xponent_format(plate_file), NA)
})
...
test_that("Parse external plates", {
  external_data_dir <- system.file("extdata", "external", package = "SerolyzerR", mustWork = TRUE)

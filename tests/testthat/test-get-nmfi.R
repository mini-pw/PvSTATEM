...
test_that("get_nmfi on artificial plate", {
  ...
  expect_no_error(plate <-
                    read_luminex_data(path, format = "xPONENT", verbose = FALSE))

  nmfi <- get_nmfi(plate, reference_dilution = 1 / 50)

  reference_dilution_index <- which(plate$dilution_values == 1 / 50)

  reference_dilution_values <-
    plate$data[["Median"]][reference_dilution_index,]

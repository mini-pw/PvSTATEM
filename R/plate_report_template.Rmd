...
Plate was run on: `r if(!is.null(params$plate$plate_datetime)) params$plate$plate_datetime else "Date of running plate was not found in Luminex file"`.  
...
for (i in 1:length(params$plate$analyte_names)) {
  ...
  print(plot_standard_curve_analyte_with_model(params$plate, model, plot_legend = FALSE, plot_asymptote = FALSE, plot_dilution_bounds = FALSE))
  cat(paste0('</a>'))
}
```
</details>

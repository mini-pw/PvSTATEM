is_valid_normalisation_type <- function(normalisation_type) {
  normalisation_type %in% SerolyzerR.env$normalisation_types
}
...
#' `create_standard_curve_model_analyte` function documentation \link[SerolyzerR]{create_standard_curve_model_analyte} or in the Model reference \link[SerolyzerR]{Model}.
#'
#'
#'
...
#'
#' More info about the nMFI normalisation can be found in `get_nmfi` function documentation \link[SerolyzerR]{get_nmfi}.
#'
...
#' \cr \code{c(`r toString(SerolyzerR.env$normalisation_types)`)}.

...
Analyte <- R6Class(
  ...
  list(
    ...
    #' The analyte object to be joined with the current analyte object - this is rather a technical inner function and should rarely be used by the user
...
#'
#' @field location_name Location of the sample formatted in readable way - `(row_letter, column)`
#'
#' @examples
#'
    ...
    blank_adjustment = function(method = "avg", inplace = "TRUE") {
      ...
      if (method == "avg") {
        ...
        for (sample in non_blank_samples) {
          sample$data <- sample$data - agg_dataframe # subtract the aggregated values
        }
      }
    },

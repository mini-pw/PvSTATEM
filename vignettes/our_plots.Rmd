...
The default plot type is violin, but there is an option to change it to violin plot by setting the `plot_type` parameter to `boxplot`.
...
Other possible values for this parameter are `c("RAU")` and `c("MFI")`, which means that only the x or y axis is in the log scale.
There is also an option to turn off some parts of the plot by setting parameters `plot_line`, `plot_blank_mean` and `plot_rau_bounds` to `FALSE`.

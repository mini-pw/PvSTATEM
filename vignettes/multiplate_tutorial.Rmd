...
By default, it saves all the outputs into the input directory, this can be changed with the `output_dir` parameter.
We also specify the format of the input files, which is `xPONENT` in this case.
...
process_dir(base_dir, format = "xPONENT", output_dir = output_dir, merge_outputs = TRUE)
...
For detailed description of the normalised out for a single plate, we refer the reader to the vignette("Basic SerolyzeR functionalities"). For the detailed description of the reports, we refer the reader to the vignette("SerolyzeR HTML reports").

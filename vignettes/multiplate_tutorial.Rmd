...
For detailed description of the normalised output for a single plate, we refer the reader to the \code{vignette("Basic SerolyzeR functionalities", package="SerolyzeR")}. For the detailed description of the reports, we refer the reader to the \code{vignette("SerolyzeR HTML reports", package="SerolyzeR")}. Here, we will just denote that `process_dir` function also allows for automatic generation of both multiplate and singleplate reports, which can be enabled by setting `generate_reports` parameter to `TRUE`.

...
Additionally, this function supports outputs for normalised Median Fluorescence Intensity (nMFI) values. More details on this normalisation approach can be found in the `nMFI` section of this document or in the documentation of [`get_nmfi()`].

It is possible to generate a simple dataframe with the structured MFI values, by specifying the `output_type` parameter to `MFI`.
...
process_file(plate_filepath, layout_filepath, output_dir = example_dir, output_types = c("RAU", "nMFI", "MFI", "report"))
```

It saves `RAU` and `nMFI` values to the `output_dir`, working similarly to the [`process_plate`] function. Additionally it has an option to produce a single plate report, which can be enabled by adding `report` to the `output_types` parameter. The report is saved in the same directory as the output files, with the name based on the plate name. For more detailed description of the reports, we refer to the vignette \code{vignette("SerolyzeR HTML reports", package="SerolyzeR")}.
...
nMFI values of the samples can be calculated in two ways - using the `get_nmfi` function or with the `process_plate` function that also saves the output into the CSV file by setting the `output_type` parameter to `nMFI` in the `process_plate` function. By default the output will be saved as a file with the same name as the plate name but with the `_nMFI` suffix. 
...
df <- process_plate(plate, output_dir = example_dir, output_type = "nMFI")
df[1:5, 1:5]
```

...
The computed RAU values are then saved to a CSV file in a specified folder, with a specified name, which by default is based on the plate name and the normalisation type.

To get more information about the function, check `?process_plate`. 

```{r}
tmp_dir <- tempdir(check = TRUE)
df <- process_plate(plate, output_dir = tmp_dir)
colnames(df)
```
...
df <- process_plate(plate, output_dir = tmp_dir, normalisation_type = "nMFI")
df[1:5, 1:5]
```

...
Apart from the `process_plate` function, the package provides a set of functions that allow for more detailed and advanced quality control and normalisation of the data.
...
nMFI values of the samples can be calculated in two ways - using the `get_nmfi` function or with the `process_plate` function that also saves the output into the csv file by setting the `normalisation_type` parameter to `nMFI` in the `process_plate` function.

...
To limit the extrapolation error from above (values above maximum RAU value $RAU_{max}$ for the standard curve samples), we clip all predictions above $M = RAU_{max} + \text{over_max_extrapolation}$ to $M$ where `over_max_extrapolation` is user controlled parameter to the `predict` function.

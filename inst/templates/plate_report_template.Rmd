...
<style>
  ...
  .plot-grid {
    ...
    gap: 10px; /* Space between the plots */
  }
...
<script>
// JavaScript to add/remove border on click
document.addEventListener("DOMContentLoaded", function() {
  var plots = document.querySelectorAll('.plot-container a');
  
  plots.forEach(function(plot) {
    plot.addEventListener('click', function() {
      // Remove highlight from any previously clicked plot
      plots.forEach(function(p) {
        p.classList.remove('highlighted');
      });
      
      // Add highlight to the clicked plot
      plot.classList.add('highlighted');
    });
  });
});
</script>
